...
export default function index() {
  ...
  const { pickUpAddr, pickUpCords, dropAddr, dropCords } = useSelector(
    (state) => state.rides
  );
  useEffect(() => {
  ...
  const handleDragEnd = (e) => {
    ...
    fetch(
      `https://api.geoapify.com/v1/geocode/reverse?lat=${pickUpCords.lat}&lon=${pickUpCords.lng}&apiKey=a1dd45a7dfc54f55a44b69d125722fcb`
    )
      .then((res) => res.json())
      .then((data) =>
        dispatch(
          setAddress({
            inputField: data.features[0].properties.formatted,
            flag: 'pickUpAddr',
          })
        )
      );
  ...
  const handleDragEndDest = (e) => {
    ...
    fetch(
      `https://api.geoapify.com/v1/geocode/reverse?lat=${dropCords.lat}&lon=${dropCords.lng}&apiKey=a1dd45a7dfc54f55a44b69d125722fcb`
    )
      .then((res) => res.json())
      .then((data) =>
        dispatch(
  ...
  const FloatBtn = (props) => (
    <>
      ...
      >
        <FloatButton
          onClick={() => props.setSelectedVehicle('car')}
          icon={<CarTwoTone />}
        />
        <FloatButton
          onClick={() => props.setSelectedVehicle('bike')}
          icon={<CommentOutlined />}
        />
      </FloatButton.Group>
    </>
  ...
  const MapView = () => {
    const { dropAddr, dropCords, pickUpAddr, pickUpCords } = useSelector(
      (state) => state.rides
    );
    ...
    const distance = getDistance(pickUpCords, dropCords);
    // const handleDestChange = ()=> {
    //   dispatch(setAddress({inputField: destRef.current.value, flag:'dropAddr'}))
    // }

    const { pricePerUnitKm, basePrice, nightPricePercentile } =
      priceMapping[selectedVehicle.toLowerCase()];
    ...
    const [estimatedPrice, setEstimatedPrice] = useState(
      Math.ceil(initialPrice)
    );
    const [fixedEstimatedPrice, setFixedEstimatedPrice] = useState(
      Math.ceil(initialPrice)
    );
    ...
    const UserRideForm = () => {
      return (
        <div>
          <Tabs
            onChange={(text) => setSelectedVehicle(text)}
            defaultActiveKey="1"
            items={vehiclesItems}
            style={{
              background: 'white',
              ...
              <div className="pr-4 ">
                <>
                  ...
                  {isLoaded && (
                    <>
                      <form>
                        <Autocomplete onPlaceChanged={handleDestChange} key={1}>
                          <input
                            type="text"
                ...
                <div>
                  <p className="mt-2 mb-2">Distance: {distance / 1000} km</p>
                  ...
                  <div onClick={() => setIsEdit(true)} className="mt-4 mb-4">
                    ...
                    <div className="mt-2 flex gap-5 justify-center align-top">
                      <button
                        ...
                        onClick={() => {
                          if (estimatedPrice <= Math.ceil(initialPrice) - 50)
                            return;
                      ...
                      <div className="flex gap-1 hover:cursor-pointer ">
                        NPR
                        <div
                          className={styles.offer_input}
                          onBlur={handleSetChangePrice}
                      ...
                      <button
                        ...
                        className=" px-3 py-1   rounded-lg bg-black text-white border-2 hover:border-green-600"
                      >
                        +10{' '}
                      </button>
                    </div>
  ...
  const handlePickUpChange = () => {
    dispatch(
      setAddress({ inputField: pickUpRef.current.value, flag: 'pickUpAddr' })
    );
  };
  const handleDestChange = () => {
    dispatch(
      setAddress({ inputField: dropRef.current.value, flag: 'dropAddr' })
    );
  };
  ...
  return (
    <>
      <div
        className="w-screen h-10vh bg-black flex justify-between p-5 "
        style={{
          borderBottom: '5px solid green ',
          borderRadius: '2px',
        }}
      >
        <div className="w-28 flex hover:cursor-pointer">
          <CustomLogo />
        </div>
        <div className="">
          <Popover
